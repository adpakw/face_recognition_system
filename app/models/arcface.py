...
class ArcFaceClient:
    """
    ArcFace model class with robust device handling
    """

    def __init__(self, weights_path: str = "app/models/weights/backbone.pth"):
        self.device = self._get_device()

        self.model = _iresnet("iresnet50", IBasicBlock, [3, 4, 14, 3])
    ...
    def _get_device(self) -> torch.device:
        """Определяет доступное устройство с проверкой доступности CUDA"""
    ...
    def _load_weights(self, weights_path: str):
        """Безопасная загрузка весов с обработкой устройств"""
        try:
    ...
    def preprocess(self, img: np.ndarray) -> torch.Tensor:
        """Препроцессинг изображения с проверкой устройства"""
    ...
    def get_embeddings(self, img: np.ndarray) -> List[float]:
        """Получение эмбеддингов с обработкой ошибок"""
        try:
            with torch.no_grad():
            ...
            raise RuntimeError(f"Embedding extraction failed: {str(e)}") from e

    def _normalize_embedding(self, embedding: torch.Tensor) -> List[float]:
        """L2-нормализация с проверкой устройства"""
        norm = torch.norm(embedding, p=2, dim=1, keepdim=True)
        normalized = embedding / (norm + 1e-10)
